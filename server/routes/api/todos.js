...
router.get("/:user/:haus", async (req, res) => {
  try {
    ...
    const todosArr = todos.todos.filter(todo => todo.haus === req.params.haus);
...
router.patch("/edit/:user/:haus/:todoid", async (req, res) => {
  const todos = await User.findById(req.params.user, "todos");
  const updatedTodo = todos.todos.filter(todo => todo._id == req.params.todoid);
...
router.delete("/:user/:haus/:todoid", async (req, res) => {
  const todos = await User.findById(req.params.user, "todos");
  const todosArr = todos.todos.filter(todo => todo.haus === req.params.haus);
  const toDelete = todosArr.filter(todo => todo._id == req.params.todoid);
